...
function ProjectWindowCard({
  ...
  const pattern = PREVIEW_PATTERNS[(index + 2) % PREVIEW_PATTERNS.length];
  const publishedCount = project.posts.filter((p) => p.isPublished).length;
    ...
    >
      ...
      <div className="border-2 border-ink bg-canvas" style={{ borderRadius: 0 }}>
        ...
        <div className="border-t-2 border-ink px-3 py-3">
          <h2 className="font-serif font-bold text-ink text-base leading-tight mb-1.5 line-clamp-1">
            {project.title}
          </h2>
          <p className="font-mono text-[8px] uppercase tracking-widest text-ink opacity-50 line-clamp-2 leading-relaxed">
            {project.description}
          </p>
          <time className="font-mono text-[8px] uppercase tracking-widest text-ink opacity-30 mt-2 block">
...
export function ProjectGrid({ projects }: { projects: ProjectWithPosts[] }) {
  const { selectedProject, selectProject } = usePageTransition();
  ...
  return (
    <div
      className="fixed left-0 right-0 overflow-hidden bg-canvas"
      style={{ top: NAV_H, bottom: FOOTER_H }}
    >
      ...
      {/* Window cards — client only */}
      {mounted && projects.map((project, index) => (
        <ProjectWindowCard
          key={project.id}
          project={project}
          index={index}
          zIndex={zMap[project.id] ?? 10 + index}
          hidden={hidden.has(project.id) || selectedProject?.project.id === project.id}
          onRaise={() => raise(project.id)}
          onClose={() => setHidden((prev) => new Set(prev).add(project.id))}
          onOpen={() => selectProject({ project, styleIndex: 0 })}
        />
      ))}
